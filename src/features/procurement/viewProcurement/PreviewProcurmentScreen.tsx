...
const PreviewProcurementScreen = ({ navigation, route }: RootNavigationProps<"ViewProcurementSummary">) => {
        ...
        <View style={tailwind("h-[400px]")}>
...

          {
            data.status == "Submitted"
              ?
              <InfoDisplayLink placeholder="Purchase Order" info={data.purchase_order_secondary_id} linkOnPress={() => { linkTo(`/purchase-orders/${data.purchase_order_id}/preview`) }} />
              :
              null
          }
...
export default PreviewProcurementScreen;