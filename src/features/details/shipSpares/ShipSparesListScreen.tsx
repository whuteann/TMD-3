...
import ShipSpareCard from '../../../components/templates/details/shipSpares/index/ShipSpareCard';
...
const ShipSpareListScreen = ({ navigation }: RootNavigationProps<"ShipSpareList">) => {
  ...
  const { data, mutate } = useCollection(SHIP_SPARES, {
    limit: LIMIT,
    where: ["deleted", "==", false],
  ...
  const onPaginate = async () => {
    ...
    const moreDocs = await shipSpareRef
      .where("deleted", "==", false)
      .orderBy("created_at", "desc")
  ...
  return (
    <Body
      header={<HeaderStack title={"Ship Spares"} navigateProp={navigation} />} 
      style={tailwind("mt-6")}
      onRefresh={onRefresh}>
      ...
      <View style={ tailwind('w-full mt-3') }>
        <FlatList
          ...
          renderItem={({ item }: { item: any }) => <ShipSpareCard navigate={() => { navigation.navigate("EditShipSpare", { docID: item.id }) }} name={item.group} description={item.product_description} code={item.product_code} />}
        />
      </View>
    </Body>
  )
...
export default ShipSpareListScreen;