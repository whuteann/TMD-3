...
import { createPaymentTermSales, deletePaymentTermSales, updatePaymentTermSales } from '../../../services/SalesPaymentTermServices';

const formSchema = Yup.object().shape({
  name: Yup.string().required("Required"),
});

const SalesPaymentTermFormScreen = ({ navigation, route }: RootNavigationProps<"CreateSalesPaymentTerm" | "EditSalesPaymentTerm">) => {
  ...
  const { data: payment_term } = useDocument<PaymentTerm>(`${SALES_PAYMENT_TERMS}/${docID}`, {
  ...
  const onSubmit = (values: any) => {
    ...
    if (docID) {
      ...
      return updatePaymentTermSales(docID, payment_term?.name || '', values, user!, UPDATE_ACTION, () => {
        ...
        revalidateCollection(SALES_PAYMENT_TERMS);
      }, (error) => {
        setLoading(false);
        setError(error);
      });
    ...
    return createPaymentTermSales(values, user!, () => {
      setLoading(false);
      setModalVisible(true);
  ...
  const handleDelete = () => {
    ...
    return deletePaymentTermSales(docID, user, () => {
      setLoading(false);
      setModalVisible(true);
      revalidateCollection(SALES_PAYMENT_TERMS);
    }, (error) => {
  ...
  const onCloseModal = () => {
    ...
    navigation.navigate("SalesPaymentTermList");
  }
          ...
          <View style={tailwind('mt-4')}>
            ...
            <View style={tailwind('flex flex-row flex-wrap')}>
              <RegularButton
                type="secondary"
                text="Cancel"
                operation={() => { navigation.navigate("SalesPaymentTermList") }} />
...
export default SalesPaymentTermFormScreen;