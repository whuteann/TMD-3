...
const SalesPaymentTermListScreen = ({ navigation }: RootNavigationProps<"SalesPaymentTermList">) => {
  ...
  const { data, mutate } = useCollection<PaymentTerm>(SALES_PAYMENT_TERMS, {
    limit: LIMIT,
    where: ['deleted', '==', false],
  ...
  const onPaginate = async () => {
    ...
    const moreDocs = await salesPaymentTermRef
      .where("deleted", "==", false)
      .orderBy("created_at", "asc")
  ...
  return (
    <Body
      header={<HeaderStack title={"Sales Payment Term"} navigateProp={navigation} />}
      style={tailwind("mt-6")}
      onRefresh={onRefresh}>

      <View style={tailwind('mb-3')}>
        <PlusButton text="Add New Sales Payment Term" operation={() => { navigation.navigate("CreateSalesPaymentTerm") }} />
      </View>
      ...
      <View style={tailwind('w-full mt-3')}>
        <FlatList
          ...
          renderItem={({ item }: { item: any }) => <PaymentTermCard navigate={() => { navigation.navigate("EditSalesPaymentTerm", { docID: item.id }) }} title={item.name} />}
        />
      </View>
...
export default SalesPaymentTermListScreen;