...
const PreviewInvoiceSummaryScreen = ({ navigation, route }: RootNavigationProps<"ViewInvoiceSummary">) => {
  ...
  return (
    <Body header={<HeaderStack title={"View Invoice"} navigateProp={navigation} />} style={tailwind("pt-10")}>
      ...
      <View>
        ...
        <InfoDisplay placeholder="Barging Fee" info={`${addCommaNumber(data.barging_fee, "-")}${data.barging_remark ? `/${data.barging_remark}` : ""}`} />
        <InfoDisplay placeholder="Discount" info={addCommaNumber(data.discount, "-")} />
        <InfoDisplay placeholder="Total" info={addCommaNumber(data.total_payable, "-")} />
...
export default PreviewInvoiceSummaryScreen;